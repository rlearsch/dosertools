...
def truncate_data(dataset : pd.DataFrame, before: bool = True) -> pd.DataFrame:
    ...
    dataset : pd.DataFrame
        ...
        Dataframe must contain "R/R0" column.
    ...
    # Raises error if dataset does not have an "R/R0" column.
...
def add_strain_rate(dataset : pd.DataFrame) -> pd.DataFrame:
    ...
    # Calculates the strain rate as -2*(d(R/R0)/dt)/(R/R0).
...
def add_critical_time(dataset : pd.DataFrame, optional_settings: dict = {}) -> pd.DataFrame:
    ...
    in radius specified by tc_bounds. Adds the columns "tc (s)" (critical time),
    ...
    # Finds indices for R/R0 corresponding to tc_bounds.
    ...
    # Finds values of critical time and ratio of radius at critical time to the
    # initial radius.
    tc = subset.at[tc_index,"time (s)"]
    Rtc = subset.at[tc_index, "R/R0"]

    # Adds tc, t-tc, and Rtc/R0 columns to the dataset.
    dataset["tc (s)"] = tc
